# Quake Live Server Standards - Capture the Flag

This Docker image contains the standard configuration for Capture the Flag as defined by [Quake Live Server Standards](https://github.com/quakelive-server-standards).
...
- `/home/steam/ql/baseq3/mappool.txt` - [Standard Capture the Flag map pool](https://github.com/quakelive-server-standards/server-framework/blob/master/mappools/standard/ctf/mappool.txt)
- `/home/steam/ql/baseq3/workshop.txt`- [Standard Capture the Flag Steam workshop items](https://github.com/quakelive-server-standards/server-framework/blob/master/workshop/standard/ctf/workshop.txt)
- `/home/steam/ql/baseq3/scripts/ctf.factories` - [Standard Capture the Flag game type definitions](https://github.com/quakelive-server-standards/server-framework/blob/master/factories/standard/ctf/ctf.factories)
- `/home/steam/ql/minqlx-plugins` - [Standard Capture the Flag minqlx plugins](https://github.com/quakelive-server-standards/server-framework/tree/master/minqlx-plugins/standard/ctf)
...
The name which is displayed in the Quake Live server browser. The default value is `Quake Live Server Standards - Capture the Flag`.

### `SV_TAGS`

The tags with which a server can be found in the Quake Live server browser. The default value is `ctf`.

### `SERVERSTARTUP`

Can be used to setup the map which the server should run after startup. Its default value is `startRandomMap`. Set a specific map by using a value like `map spidercrossings stdctf`.
